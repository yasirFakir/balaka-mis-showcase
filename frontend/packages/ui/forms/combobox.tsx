...
function ComboboxInput({
  ...
  return (
    <InputGroup className={cn("w-full h-11", className)}>
      <ComboboxPrimitive.Input
        render={<InputGroupInput disabled={disabled} className="h-full" />}
        {...props}
      />
    ...
    <ComboboxPrimitive.Portal>
      ...
      >
        <ComboboxPrimitive.Popup
          ...
          data-chips={!!anchor}
          className={cn("bg-white text-primary data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 data-closed:zoom-out-95 data-open:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 border-2 border-primary min-w-36 overflow-hidden rounded-none shadow-none duration-100 group/combobox-content relative max-h-(--available-height) w-(--anchor-width) max-w-(--available-width) min-w-[calc(var(--anchor-width)+--spacing(7))] origin-(--transform-origin) data-[chips=true]:min-w-(--anchor-width) z-[1000] p-1", className )}
          {...props}
        />
      </ComboboxPrimitive.Positioner>
    </ComboboxPrimitive.Portal>
...
function ComboboxItem({
  ...
  return (
    <ComboboxPrimitive.Item
      ...
      className={cn(
        "data-highlighted:bg-secondary/25 data-highlighted:text-primary data-highlighted:font-bold data-highlighted:border-l-[6px] data-highlighted:border-primary gap-2 rounded-none py-2 pr-8 pl-2 text-sm font-medium text-primary/90 [&_svg:not([class*='size-'])]:size-4 relative flex w-full cursor-default items-center outline-hidden select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 transition-all",
        className
      )}
      {...props}
    >
