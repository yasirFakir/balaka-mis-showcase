...
export function DynamicForm({ schema, onSubmit, onValuesChange, submitLabel = "Submit Application", defaultValues: initialValues, context }: DynamicFormProps) {
  ...
  const loadRemoteOptions = async (source: string): Promise<string[]> => {
    try {
        ...
        if (source === "staff") {
            ...
            const users = Array.isArray(data) ? data : (data.items || []);
            options = users
                .filter(u => u.roles.some((r: any) => r.name === "Client"))
